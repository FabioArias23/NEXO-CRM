...
import { 
  Loader2, GripVertical, Building2, User, Calendar, 
  Plus, Search, Filter, X, Users, DollarSign, TrendingUp, Edit2
} from 'lucide-react';
...
function OpportunityCard({ opportunity, onClick, onQuickEdit }: OpportunityCardProps) {
  ...
  return (
    <div
      ...
      // CAMBIO: Fondo blanco (bg-card) y sombra suave para resaltar sobre la columna gris
    ...
    >
      <div className="flex flex-col gap-2">
        <div className="flex items-start justify-between">
           <h4 className="text-sm font-semibold text-foreground truncate flex-1 pr-2">{opportunity.name}</h4>
           <button
             onClick={onQuickEdit}
             className="opacity-0 group-hover:opacity-100 p-1 hover:bg-accent rounded transition-all -mt-1 -mr-1"
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border/50 mt-1">
          <span className="text-xs font-bold text-foreground bg-accent/50 px-1.5 py-0.5 rounded">
            ${opportunity.value.toLocaleString()}
          </span>
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
             <Calendar className="w-3 h-3" />
             <span>{new Date(opportunity.closeDate).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' })}</span>
          </div>
        </div>
      </div>
    </div>
...
function StageColumn({ stage, opportunities, onDrop, onCardClick, onQuickEdit, onAddNew }: StageColumnProps) {
    ...
    <div className="flex-shrink-0 w-80 h-full flex flex-col">
      ...
      */}
      ...
      >
        ...
        <div className={`p-3 bg-background/50 border-b border-border backdrop-blur-sm sticky top-0 z-10 border-t-4 ${getStageAccentColor(stage)}`}>
          ...
          <div className="flex items-center justify-between">
             ...
             <span className="text-xs font-mono font-medium text-foreground opacity-80">${totalValue.toLocaleString()}</span>
          </div>
        </div>
...
export function KanbanView() {
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    if ((!isAdmin || filterByUser) && user) {
      filtered = filtered.filter(opp => opp.ownerId === user.id);
    }
  ...
  return (
    <>
      <DndProvider backend={HTML5Backend}>
        <div className="p-6 h-full flex flex-col bg-background text-foreground transition-colors duration-300">
          
          {/* Header Superior */}
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-foreground mb-1">Pipeline Kanban</h1>
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                 <span className="flex items-center gap-1.5 px-2 py-1 bg-secondary rounded-md">
                    <DollarSign className="w-3.5 h-3.5" /> 
                    <span className="font-mono text-foreground font-medium">${totalPipelineValue.toLocaleString()}</span> Total
                 </span>
                 <span className="flex items-center gap-1.5 px-2 py-1 bg-secondary rounded-md">
                    <TrendingUp className="w-3.5 h-3.5" /> 
                    <span className="font-mono text-foreground font-medium">${Math.round(weightedValue).toLocaleString()}</span> Pond.
                 </span>
                 <span className="flex items-center gap-1.5 px-2 py-1 bg-secondary rounded-md">
                    <Users className="w-3.5 h-3.5" /> 
                    <span className="font-mono text-foreground font-medium">{filteredOpportunities.length}</span> Deals
                 </span>
              </div>
            </div>
              ...
              {isAdmin && (
                <button
                  ...
                  className={`p-2 rounded-lg transition-colors border ${
                    showFilters ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-input hover:bg-accent text-muted-foreground'
                  }`}
                >
          ...
          {showFilters && isAdmin && (
            <div className="bg-card border border-border rounded-xl p-3 mb-4 shadow-sm animate-in slide-in-from-top-1">
              <label className="flex items-center gap-2 cursor-pointer w-fit p-1.5 hover:bg-accent rounded-lg transition-colors">
                ...
                />
                <span className="text-sm font-medium text-foreground">Mostrar solo mis oportunidades</span>
              </label>
            </div>
          )}
