...
import { ColumnSelector, ColumnConfig } from '../components/table/ColumnSelector';
import { AdvancedFilters, FilterRule } from '../components/table/AdvancedFilters';
import { QuickStats } from '../components/table/QuickStats';
import { Search, ArrowUpDown, User, Clock, Loader2, Filter, Download } from 'lucide-react';
...
export function TableView() {
  ...
  }, [opportunities, searchQuery, stageFilter, advancedFilters, sortField, sortDirection]);
    ...
    <>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl text-white mb-1">Vista de Tabla</h1>
            <p className="text-gray-500">{filteredOpportunities.length} oportunidades totales</p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 flex items-center gap-3">
              ...
              <select
                ...
                className="px-4 py-2 bg-black border border-gray-800 rounded-lg text-white focus:outline-none focus:border-gray-600"
              >
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className={`p-2 border border-gray-800 rounded-lg hover:bg-gray-900 transition-colors ${
                  showAdvancedFilters ? 'bg-gray-900 text-white' : 'text-gray-400 hover:text-white'
                }`}
              >
              ...
              <thead className="bg-gray-900 border-b border-gray-800">
                <tr>
                  {columns.filter(col => col.visible).map(col => (
                    <th key={col.key} className="px-6 py-4 text-left text-sm text-gray-400">
                      {col.label}
                    </th>
                  ))}
                </tr>
                  ...
                  filteredOpportunities.map((opp) => (
                    ...
                    >
                        ...
                        <td className="px-6 py-4">
                          <div>
                            ...
                            <p className="text-xs text-gray-600">
                              Cierre: {new Date(opp.closeDate).toLocaleDateString('es-ES')}
                            </p>
                          </div>
                        </td>
                      )}
                      {columns.find(c => c.key === 'value')?.visible && (
                        <td className="px-6 py-4">
                          <div>
                            <p className="text-sm text-white">${opp.value.toLocaleString()}</p>
                            <p className="text-xs text-gray-600">{opp.probability}% prob.</p>
                          </div>
                        </td>
                      )}
                      {columns.find(c => c.key === 'stage')?.visible && (
                        <td className="px-6 py-4">
                          <span className={`inline-block px-3 py-1 text-xs rounded ${getStageColor(opp.stage)}`}>
                            {opp.stage}
                          </span>
                        </td>
                      )}
                      {columns.find(c => c.key === 'company')?.visible && (
                        <td className="px-6 py-4 text-sm text-gray-300">{opp.company}</td>
                      )}
                      {columns.find(c => c.key === 'contact')?.visible && (
                        <td className="px-6 py-4 text-sm text-gray-300">{opp.contact}</td>
                      )}
                      {columns.find(c => c.key === 'owner')?.visible && (
                        <td className="px-6 py-4">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-sm text-gray-300">{opp.ownerName}</p>
                              <p className="text-xs text-gray-600">{opp.ownerEmail}</p>
                            </div>
                          </div>
                        </td>
                      )}
                      {columns.find(c => c.key === 'modified')?.visible && (
                        <td className="px-6 py-4">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-sm text-gray-300">{opp.lastModifiedByName}</p>
                              <p className="text-xs text-gray-600">
                                {new Date(opp.updatedAt).toLocaleString('es-ES', {
                                  month: 'short',
                                  day: 'numeric',
                                  hour: '2-digit',
                                  minute: '2-digit',
                      ...
                      {columns.find(c => c.key === 'probability')?.visible && (
                        <td className="px-6 py-4 text-sm text-gray-300">{opp.probability}%</td>
                      )}
