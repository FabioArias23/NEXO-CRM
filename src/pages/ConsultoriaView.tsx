...
import { 
  Search, Plus, ChevronDown, Filter, ArrowUpDown, 
  Eye, Download, List, Grid3X3, Table as TableIcon,
  Calendar, Phone, User, CheckSquare, AlignLeft, MoreHorizontal,
  Check, MessageCircle, Menu, X, ChevronRight
} from 'lucide-react';
import { RecordDrawer, ClientRecord } from '../components/modals/RecordDrawer';
...
const ALL_COLUMNS: ColumnDef[] = [
  { key: 'fechaCarga', label: 'FECHA CARGA', icon: Calendar, width: 'w-32', minWidth: 120 },
  { key: 'caller', label: 'CALLER', icon: User, width: 'w-32', minWidth: 120 },
  { key: 'derivados', label: 'DERIVADOS', icon: CheckSquare, width: 'w-24', minWidth: 100 },
  { key: 'razonSocial', label: 'RAZÓN SOCIAL', icon: AlignLeft, width: 'w-64', minWidth: 250 },
  { key: 'contacto', label: 'CONTACTO', icon: User, width: 'w-48', minWidth: 200 },
  { key: 'tel1', label: 'TELÉFONO', icon: Phone, width: 'w-40', minWidth: 160 },
  { key: 'status', label: 'STATUS', icon: MoreHorizontal, width: 'w-40', minWidth: 160 },
];

// --- DATOS MOCK ---
const INITIAL_MOCK_DATA: ClientRecord[] = [
  { id: 1, fechaCarga: '2025-12-02', caller: 'Juan', derivados: '', razonSocial: 'PREVENCION ART', contacto: 'AGUILERA CLAUDIA', tel1: '1155000012', status: 'Contactar Luego', observaciones: 'Interesado en cotización básica.' },
  { id: 2, fechaCarga: '2025-02-12', caller: 'Juan', derivados: 'Si', razonSocial: 'OMINT ART', contacto: 'MELANY MORRO', tel1: '11 15-7105-4844', status: 'Contactar Luego', observaciones: 'Llamar después de las 14hs.' },
  { id: 3, fechaCarga: '2025-12-04', caller: 'Juan', derivados: '', razonSocial: 'EXPERTA ART', contacto: 'GIMENEZ BRIAN', tel1: '1166319605', status: 'Nuevo', observaciones: '' },
  { id: 4, fechaCarga: '2025-12-10', caller: 'Florencia', derivados: '', razonSocial: 'FEDERACION PATRONAL', contacto: 'CHAVEZ NAHUEL', tel1: '011 15 7151-9007', status: 'Nuevo', observaciones: 'Preguntar por gerente de RRHH.' },
  { id: 5, fechaCarga: '2025-12-23', caller: 'Cecilia', derivados: '', razonSocial: 'ANDINA ART', contacto: 'ALONSO LUCAS', tel1: '1130521890', status: 'Cerrado', observaciones: 'Venta cerrada exitosamente.' },
  { id: 6, fechaCarga: '2025-12-17', caller: 'Juan', derivados: 'No', razonSocial: 'PREVENCION ART', contacto: 'LEGUIZA ENZO', tel1: '11-76245483', status: 'Contactado', observaciones: 'Enviado email con propuesta.' },
  { id: 7, fechaCarga: '2025-12-22', caller: 'Miriam', derivados: 'Si', razonSocial: 'FEDERACION PATRONAL', contacto: 'CASTAÑO BRAIAN', tel1: '+54 9 11 2489 7373', status: 'Contactar Luego', observaciones: '' },
];
...
const VIEWS_CONFIG: ViewConfig[] = [
  { id: 'all', name: 'Base General', icon: Grid3X3, filters: {}, description: 'Todos los registros.', groupBy: 'status' },
  { id: 'juan', name: 'Vista Juan', icon: TableIcon, filters: { caller: 'Juan' }, groupBy: 'status', description: 'Mis clientes.' },
  { id: 'florencia', name: 'Vista Florencia', icon: TableIcon, filters: { caller: 'Florencia' }, groupBy: 'status', description: 'Clientes de Florencia.' },
  { id: 'cecilia', name: 'Vista Cecilia', icon: TableIcon, filters: { caller: 'Cecilia' }, groupBy: 'status', description: 'Clientes de Cecilia.' },
];

export function ConsultoriaView() {
  // Estados
  const [records, setRecords] = useState<ClientRecord[]>(INITIAL_MOCK_DATA);
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof ClientRecord, direction: 'asc' | 'desc' } | null>(null);
  const [visibleColumns, setVisibleColumns] = useState<string[]>(ALL_COLUMNS.map(c => c.key as string));
  const [showColumnMenu, setShowColumnMenu] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>({});

  const activeViewConfig = useMemo(() => {
    return VIEWS_CONFIG.find(view => view.id === activeViewId) || VIEWS_CONFIG[0];
  }, [activeViewId]);
    ...
    for (const key in activeViewConfig.filters) {
      if (activeViewConfig.filters.hasOwnProperty(key)) {
        data = data.filter(record => 
          record[key as keyof ClientRecord] === activeViewConfig.filters[key as keyof ClientRecord]
        );
      }
    }
  ...
  const groupedRecords = useMemo(() => {
    if (activeViewConfig.groupBy) {
      ...
      const statusOrder = ['Nuevo', 'Contactar Luego', 'Contactado', 'Cotizando', 'Cerrado', 'Cerrado Ganado'];
      
      processedRecords.forEach(record => {
        const key = record[activeViewConfig.groupBy!] as string || 'Sin Agrupar';
  ...
  const handleAddRecord = () => {
    ...
    if (activeViewConfig.filters.caller) defaultCaller = activeViewConfig.filters.caller as string;
  ...
  return (
    ...
    <div className="flex flex-col h-full bg-background text-foreground font-sans overflow-hidden transition-colors duration-300">
      ...
      <div className="h-14 md:h-12 border-b border-border flex items-center justify-between px-4 bg-background flex-shrink-0">
        <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
          ...
          <div className="flex items-center gap-2 cursor-pointer hover:bg-accent px-2 py-1 rounded truncate transition-colors">
             <h1 className="font-bold text-sm tracking-wide truncate">CONSULTORA MF 2</h1>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
           <span className="text-xs text-muted-foreground mr-2 hidden sm:block">Guardado</span>
      ...
      <div className="h-10 border-b border-border flex items-center px-2 bg-background overflow-x-auto scrollbar-hide flex-shrink-0">
         ...
         <button onClick={handleAddRecord} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground px-3 py-1 border-l border-border ml-2 whitespace-nowrap transition-colors">
        ...
        <aside className="w-60 border-r border-border bg-card flex flex-col flex-shrink-0 hidden md:flex">
           ...
           <div className="flex-1 overflow-y-auto px-2 space-y-4">
              <div>
                 ...
                 <ViewItem label="Vista Florencia" icon={Grid3X3} active={activeViewId === 'florencia'} onClick={() => setActiveViewId('florencia')} />
                 <ViewItem label="Vista Cecilia" icon={Grid3X3} active={activeViewId === 'cecilia'} onClick={() => setActiveViewId('cecilia')} />
              </div>

              <div>
                 <p className="px-2 text-[10px] font-bold text-muted-foreground uppercase mb-1">Vistas Personales</p>
              ...
              <div className="flex items-center gap-2 pr-4">
                 ...
                 <div className="relative">
                   ...
                   >
                      ...
                      <span className="hidden sm:inline">{ALL_COLUMNS.length - visibleColumns.length > 0 ? `${ALL_COLUMNS.length - visibleColumns.length} ocultos` : 'Ocultar campos'}</span>
                   ...
                   {showColumnMenu && (
                     <>
                       ...
                       <div className="absolute top-full left-0 mt-1 w-56 bg-popover border border-border rounded-lg shadow-xl z-20 p-2 flex flex-col gap-1 max-h-64 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                          <p className="text-[10px] text-muted-foreground uppercase px-2 py-1">Mostrar columnas</p>
                          {ALL_COLUMNS.map(col => (
                            <button 
                              key={col.key as string}
                              onClick={() => toggleColumnVisibility(col.key as string)}
                            ...
                            >
                              <div className={`w-3 h-3 rounded-sm border flex items-center justify-center ${visibleColumns.includes(col.key as string) ? 'bg-blue-600 border-blue-600' : 'border-muted-foreground'}`}>
                                {visibleColumns.includes(col.key as string) && <Check className="w-2 h-2 text-white" />}
                              </div>
                              {col.label}
                            </button>
                          ))}
                 ...
                 <ToolButton icon={Filter} label="Filtrar" active={searchTerm !== ''} />
                 <ToolButton icon={List} label="Agrupar" active={!!activeViewConfig.groupBy} />
                 <ToolButton icon={ArrowUpDown} label="Ordenar" onClick={() => toggleSort('fechaCarga')} active={!!sortConfig} />
                 ...
                 <thead className="sticky top-0 z-10 bg-background border-b border-border shadow-sm">
                    <tr>
                       <th className="w-10 border-r border-border p-2 text-center bg-background sticky left-0 z-20">
                         <input type="checkbox" className="rounded bg-secondary border-muted-foreground" />
                       </th>
                       {ALL_COLUMNS.filter(c => visibleColumns.includes(c.key as string)).map(col => (
                         <HeaderCell 
                            key={col.key as string} 
                            icon={col.icon} 
                            label={col.label} 
                 ...
                 <tbody>
                    {groupedRecords.map(group => (
                      <React.Fragment key={group.groupName}>
                        <tr className="bg-muted/30 sticky left-0 z-10">
                           <td colSpan={visibleColumns.length + 2} className="p-0">
                             <GroupHeader 
                               ...
                               color={
                                 group.groupName === 'Contactar Luego' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20' :
                                 group.groupName === 'Nuevo' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' :
                                 group.groupName === 'Cerrado' ? 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20' :
                        ...
                        {!collapsedGroups[group.groupName] && group.items.map((row) => (
                           <TableRow 
                              key={row.id} 
                              row={row} 
        ...
        <div className="fixed inset-0 z-50 flex">
           ...
           <div className="relative w-64 bg-background border-r border-border flex flex-col h-full animate-in slide-in-from-left duration-200">
              ...
              <div className="p-4 space-y-4 overflow-y-auto">
                 <div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase mb-2">Mis Vistas</p>
                    {VIEWS_CONFIG.map(view => (
                        <ViewItem 
                          key={view.id} 
                          label={view.name} 
                          icon={view.icon} 
                          active={activeViewId === view.id} 
                          onClick={() => { setActiveViewId(view.id); setIsMobileMenuOpen(false); }} 
                        />
                    ))}
                 </div>
              </div>
...
function TabButton({ children, active }: { children: React.ReactNode, active?: boolean }) {
  return (
    <button className={`px-3 py-1.5 rounded-t-lg text-xs font-medium transition-colors whitespace-nowrap ${active ? 'bg-background text-foreground border-t border-x border-border translate-y-[1px]' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}`}>
      {children}
    </button>
  );
}
...
function ViewTab({ label, active, onClick }: any) {
   return (
      <div onClick={onClick} className={`flex items-center gap-1 px-4 py-2 text-xs font-medium border-r border-border cursor-pointer min-w-max transition-colors ${active ? 'bg-accent text-foreground' : 'text-muted-foreground hover:bg-accent/50'}`}>
...
function ViewItem({ label, icon: Icon, active, onClick }: any) {
   return (
      <div onClick={onClick} className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer group transition-colors ${active ? 'bg-accent text-foreground' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}`}>
         <Icon className={`w-4 h-4 ${active ? 'text-blue-500' : 'text-muted-foreground group-hover:text-foreground'}`} />
...
function ToolButton({ icon: Icon, label, active, onClick }: any) {
   return (
      <button onClick={onClick} className={`flex items-center gap-1.5 px-2 py-1 rounded hover:bg-accent text-xs transition-colors ${active ? 'text-blue-500 bg-blue-500/10' : 'text-muted-foreground'}`}>
...
function HeaderCell({ icon: Icon, label, width, minWidth, onClick, isSorted }: any) {
   return (
      <th 
        onClick={onClick}
      ...
      >
         <div className="flex items-center gap-2">
            <Icon className={`w-3.5 h-3.5 ${isSorted ? 'text-blue-500' : 'text-muted-foreground group-hover:text-foreground'}`} />
            <span className={`truncate uppercase tracking-wide ${isSorted ? 'text-blue-500' : ''}`}>{label}</span>
            {isSorted && <ArrowUpDown className="w-3 h-3 ml-auto text-blue-500" />}
            {!isSorted && <ChevronDown className="w-3 h-3 ml-auto opacity-0 group-hover:opacity-100" />}
         </div>
      </th>
   )
}
...
const GroupHeader = ({ label, count, color, isExpanded, onToggle }: any) => (
  ...
  >
    ...
    <span className="text-[11px] text-muted-foreground font-medium ml-1">({count} registros)</span>
  </div>
);
...
function TableRow({ row, visibleColumns, onClick, onUpdate }: any) {
  const handleWhatsAppClick = (e: React.MouseEvent, phone: string) => {
    ...
    else if (cleanNumber.length === 12 && cleanNumber.startsWith('54')) cleanNumber = `549${cleanNumber.slice(2)}`;
  ...
  const renderCell = (colKey: string) => {
    switch(colKey) {
      case 'fechaCarga': return <span className="font-mono text-muted-foreground">{row.fechaCarga}</span>;
      case 'caller': return (
      ...
      case 'derivados': return row.derivados && <CheckSquare className="w-3.5 h-3.5 text-green-500 inline" />;
      case 'razonSocial': return <span className="font-medium text-foreground truncate block" title={row.razonSocial}>{row.razonSocial}</span>;
      case 'contacto': return <span className="text-foreground/80 truncate block" title={row.contacto}>{row.contacto}</span>;
      case 'tel1': return (
         <div className="flex items-center justify-between w-full group/cell h-full min-h-[20px]">
            {isEditingTel1 ? (
              <input 
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') handleBlurTel1(); }}
                autoFocus
              ...
              <>
                <span 
                    onClick={(e) => { e.stopPropagation(); setIsEditingTel1(true); }}
        ...
        <div className="w-full h-full flex items-center">
            {isEditingStatus ? (
              <select
                ...
                onClick={(e) => e.stopPropagation()} 
              >
                {['Nuevo', 'Contactar Luego', 'Contactado', 'Cotizando', 'Cerrado', 'Cerrado Ganado', 'Cerrado Perdido'].map(status => (
              ...
              <span 
                ...
                className={`px-2 py-0.5 rounded text-[10px] border cursor-pointer hover:opacity-80 transition-opacity whitespace-nowrap font-medium tracking-wide ${
                    row.status === 'Contactar Luego' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20' :
                    row.status === 'Nuevo' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' :
                    row.status === 'Cerrado' ? 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20' :
                    'bg-secondary text-muted-foreground border-border'
                }`}
              >
                {row.status}
      ...
      default: return <span className="text-muted-foreground">{row[colKey]}</span>;
    }
  };
   ...
   return (
      ...
      >
         ...
         {visibleColumns.map((colKey: string) => (
           <td key={colKey} className="border-r border-border px-3 py-1 text-[12px] truncate relative h-full">
             {renderCell(colKey)}
           </td>
         ))}
