...
export function RecordDrawer({
  ...
  const [activeTab, setActiveTab] = useState<"detalle" | "historial">(
    "detalle",
  );
    ...
    <>
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-gray-800 flex justify-between items-start bg-gray-950">
          ...
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-white p-1 hover:bg-gray-800 rounded-md transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <div className="space-y-6">
              <Section title="Información de Contacto">
                <Field
                  ...
                  value={record.tel1}
                  icon={Phone}
                  onUpdate={(val) => onUpdateRecord?.(record.id, "tel1", val)}
                />
                <Field
                  label="Teléfono Secundario"
                  value={record.tel2 || "-"}
                  icon={Phone}
                  onUpdate={(val) => onUpdateRecord?.(record.id, "tel2", val)}
                />
            ...
            <div className="space-y-4">
              ...
              <div className="relative border-l border-gray-800 ml-2 space-y-6">
                <TimelineItem
                  ...
                  text="Cambió el estado a 'Contactar Luego'"
                />
                <TimelineItem
                  date="02-Dec-25"
                  user="Sistema"
                  action="Creación"
                  text="Registro importado."
                />
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-950 flex justify-end gap-3">
          ...
          <button
            onClick={handleSave}
            className="px-6 py-2 bg-white text-black rounded-lg text-xs font-bold hover:bg-gray-200 transition-colors flex items-center gap-2"
          >
...
function ActionButton({ icon: Icon, label, color, hover, onClick }: any) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center gap-1 py-2 rounded-lg border border-gray-800 bg-gray-900/50 transition-all ${hover} group active:scale-95`}
    >
      ...
      <div className="flex-1">
        {onUpdate && isEditing ? (
          ...
          >
            ...
            {onUpdate && (
              <MoreHorizontal className="w-3.5 h-3.5 text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity" />
            )}
          </div>
        )}
      </div>
