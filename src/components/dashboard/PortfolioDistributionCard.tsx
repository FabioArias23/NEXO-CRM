      ...
      <div className="flex items-center justify-between mb-2 flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <div className="p-2 bg-blue-500/10 rounded-lg border border-blue-500/20 shrink-0">
            <PieChartIcon className="w-4 h-4 shrink-0 text-blue-400" />
          </div>
      ...
      <div className="grid grid-cols-2 gap-6 flex-1 items-center min-h-0">
        ...
        <div className="space-y-3 flex flex-col justify-center pl-2">
          {data.map((item, i) => (
            <div
              key={i}
              className="flex items-center justify-between group w-full cursor-default border-b border-border pb-2 last:border-0 last:pb-0"
            >
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                ></div>
                <span className="text-xs text-muted-foreground font-medium truncate group-hover:text-card-foreground transition-colors">
                  {item.name}
                </span>
              </div>
              <span className="text-sm font-bold text-card-foreground font-mono ml-2 tabular-nums">
                {item.value}%
              </span>
            </div>
          ))}
