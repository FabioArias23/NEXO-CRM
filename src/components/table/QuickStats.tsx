...
export function QuickStats({ opportunities }: QuickStatsProps) {
  ...
  const activeCount = opportunities.filter(opp => 
    !['Cerrado Ganado', 'Cerrado Perdido'].includes(opp.stage)
  ).length;
...
function StatCard({ title, value, icon: Icon, color, bg }: any) {
  return (
    <div className="bg-card border border-border rounded-xl p-4 shadow-sm flex items-center justify-between hover:border-primary/20 transition-all">
      <div>
